...
class DiceGui(tk.Tk):
    ...
    The class DiceGUI is an inheritance of tkinter (tk.Tk) for wrapping all GUI in it's own class.
    """

    # Class static values for styling GUI widgets
    ...
    def __init__(self, script_name, data_to_gui, data_from_gui):
        self._data_to_gui = data_to_gui
        self._data_from_gui = data_from_gui
        tk.Tk.__init__(self)
        ...
        self._default_font = tkfont.Font(size=self.FONT_SIZE_DEFAULT)
        self._large_font = tkfont.Font(size=self.FONT_SIZE_LARGE)

        self._stringvar_dice = self._dice_frame()
        ...
        self.mainloop()  # Execute GUI framework

    def _dice_rolling_update(self):
        interval_timer_start = timer()
        if not self._data_to_gui.empty():  # If data to gui arrived
    ...
    def _dice_frame(self, title="Latest Dice Roll"):
        frame = tk.LabelFrame(self, text=" "+title+" ", font=self._large_font, labelanchor="n")
    ...
    def _dice_update(self, dices):
        ...
        self._stringvar_dice.set(dice_value_text)

    def _statistics_frame(self, title="Statistics"):
        statistics_frame = tk.LabelFrame(self, text=" " + title + " ", font=self._large_font, labelanchor="n")
    ...
    def _statistics_table(self, frame):
        ...
        for column in range (0, 6*DICE_AMOUNT_MAX+1):
            ...
            for row, row_header in enumerate(STATISTICS_HEADERS, 0):
                ...
                label = tk.Label(cell, font=self._default_font)
    ...
    def _statistics_rolling_info(self, frame):
        ...
        cell.grid()
        label = tk.Label(cell, text="Rollings", font=self._default_font)
        label["bg"] = self.STATISTICS_HEADER_BG
        label.grid()
        cell = tk.Frame(roll_count_frame, relief=tk.RIDGE, borderwidth=1)
        cell.grid(row=0, column=1)
        label = tk.Label(cell, textvariable=self.stringvar_rolling_counts, font=self._default_font, anchor='e')
    ...
    def _user_input_frame(self):
        ...
        self.button_start = tk.Button(input_frame, text="Start", font=self._large_font, command=self._start_dice_rolling)
        self.button_start.grid()

        self.button_stop = tk.Button(input_frame, text="Stop", font=self._large_font, command=self._stop_dice_rolling)
        self.button_stop.grid(row=0, column=1, padx=(self.PADDING_DEFAULT, 0))

        self.label_dices = tk.Label(input_frame, text="Number of dices", font=self._large_font)
        ...
        self.spinbox_dices = tk.Spinbox(input_frame, from_=1, to=DICE_AMOUNT_MAX, font=self._large_font, justify="right")
        ...
        self.spinbox_dices["selectbackground"] = self.spinbox_dices["bg"]
        self.spinbox_dices["selectforeground"] = self.spinbox_dices["fg"]
    ...
    def _start_dice_rolling(self):
        ...
        self._data_from_gui.put(data_packet)
...
class DiceRolling:
    ...
    def _rolling_generator_thread(self, data_from_gui, data_to_gui):
        NUMBER_OF_RESULTS = DICE_AMOUNT_MAX * 6  # Possible dice values are 6
    ...
    @staticmethod  # Is static, it's a supporting function to a thread in the same class
...
class DataToGui:
    ...
    Data is packed as dictionary data sets.
    """
